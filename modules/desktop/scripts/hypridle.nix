...
{ config, pkgs, ... }: let
  ...
  desktop.scripts.hypridle = {
    ...
    kill-notify = pkgs.writeShellScript "hypridle-kill-notify.sh" ''
      killer() {
        if ${pgrep} -f "${config.desktop.scripts.hypridle.send-notify}" > /dev/null; then
          ${pkill} -f "${config.desktop.scripts.hypridle.send-notify}"
        fi
      }
    ...
    lock-resume = pkgs.writeShellScript "hypridle-lock-resume.sh" ''
      ...
      ${config.desktop.scripts.hypridle.smart-hyprlock}
    '';
  };
}
